# Založenie a prevádzka malého herného štúdia

**Dôležité:** Toto nie je návod ako to má byť "správne". Je to môj pohľad založený na osobnej skúsenosti. Každý môže mať inú cestu a neexistuje overený postup, ktorý funguje pre všetkých.


## Rýchle nadobúdanie schopností

### Najdôležitejšia schopnosť pri zakladaní biznisu
- Vedieť rýchlo nadobúdať nové schopnosti je kľúčové pre všetko ostatné
- Potrebujete túto schopnosť pre: networking, technické veci, byrokraciu, vedenie tímu
- **Často potrebujete spraviť niečo, čo nikto v tíme nevie**

### Odporúčanie: "The First 20 Hours" od Josha Kaufmana
- Z tejto knihy som sa naučil, ako efektívne a rýchlo nadobúdať nové schopnosti
- Princíp: Nemusíte byť expertom, stačí sa naučiť to podstatné pre váš konkrétny cieľ

### Môj prístup
1. **Nastavte si presný cieľ** – čo konkrétne potrebujete dosiahnuť
2. **Naučte sa len nevyhnutné veci** – ktoré vás dostanú k cieľu
3. **Dosiahnite cieľ** – aplikujte naučené v praxi
4. **Postupne dopĺňajte znalosti** – stavajte na základoch

### Praktický príklad: Level design

**Situácia:** Potrebovali sme spraviť levely do našej 3D hry s pocitom nelinearity

**Proces:**
1. Pozeral som videá o 3D level designe
2. Všetky videá som prepustil cez AI transkripciu
3. S AI som diskutoval o konkrétnych riešeniach pre našu hru
4. Naučil som sa kľúčový workflow:
   - Najprv náčrt na papieri
   - Potom playable prototype (od začiatku do konca)
   - Playtest
   - Iterácia až do dosiahnutia cieľa

**Výsledok:**
- Spísal som znalosti do knowledge base
- Zdieľal som to s dizajnérmi
- Všetci sme mali rovnaký workflow
- Rýchlejší a efektívnejší vývoj

### Kľúč k úspechu
- **Nebojte sa púšťať do nových vecí**
- **Robte to efektívne** – nie náhodne, ale systematicky
- Táto schopnosť sa vám zíde vo všetkých oblastiach prevádzky štúdia


## Základné nastavenie

### Byrokracia a založenie firmy
- Založenie firmy je jednoduché – použite služby ako sroonline.sk alebo podobné
- Po založení: bankový účet v mene firmy + nájdite si účtovníka
- S týmito nákladmi počítajte v budgete

### Právne základy
- Dajte si vypracovať vzory zmlúv pre:
  - Spoluprácu so SZČO
  - Trvalý pracovný pomer
  - Spoluprácu so s.r.o.
- **Kľúčové:** Zmluvy musia obsahovať, že všetky práva na vytvorenú prácu vlastní vaša firma

### Hry vs. biznis
- Robiť hry je kreatívna, srdcová záležitosť
- Biznis vyžaduje chladnejší pohľad a triezvejšie rozhodnutia
- Niekedy musíte spraviť nepríjemné rozhodnutia – to je súčasť prevádzky štúdia

## Veľké otázky na začiatku

### Definujte si cieľ
- Chcete spraviť hru, ktorá vás baví?
- Chcete zarobiť peniaze?
- Budujete si portfólio pre budúce príležitosti?
- **Jasný cieľ = rýchlejšie a lepšie rozhodnutia**

### Publisher vs. indie cesta
Obe možnosti sú v poriadku, ale vyžadujú iný prístup:

**Cesta cez publishera:**
- Potrebujete portfólio a existujúce hry
- Musíte dokázať, že tím je silný a prešiel si produkciou
- Hra musí mať veľký appeal a premyslenú monetizáciu

**Indie cesta:**
- Musíte vyriešiť financovanie
- Aj solo developer musí počítať s rozpočtom na seba – váš čas nie je zadarmo


## Financovanie a rozpočet

### Sledovanie cash flow
- Vytvorte si tabuľku s mesačnými výdavkami a príjmami
- Vypočítajte si **runway** – ako dlho vydržíte bez ďalších príjmov
- Počítajte s rezervou (odporúčam 15-20%)
- Toto vám dá realistický odhad, čo musíte mať do kedy hotové

### Pravidelné prognózy
- Každé 2-3 mesiacov spravte nový odhad
- Porovnajte, ako presný bol váš predchádzajúci odhad
- Ak ste sa výrazne odchýlili, prehodnoťte metódu výpočtu
- Cieľ: postupne dosiahnuť dostatočne presný výpočet pre budúcnosť

### Burn rate – pomalšie je rýchlejšie

**Nenaháňajte sa s expanziou:**
- Keď dostanete budget, nepovedzte si hneď: "S týmto počtom ľudí to spravíme takto rýchlo"
- Nastavte si radšej pomalšie míňanie peňazí

**Čo prináša rýchla expanzia:**
- Musíte sa nastaviť osobnostne ako tím
- Musíte vytvoriť workflow od nuly
- Niekto musí všetkým vytvárať tasky a plánovať
- Musíte nastavovať ciele, sledovať ich plnenie a riešiť prečo sa (ne)plnia
- Musíte definovať, kto na čom robí

**Praktický príklad:**
- Niekedy zistíte, že ešte nepotrebujete pracovať na arte
- Namiesto 2-3 artistov stačí jeden, ktorý pracuje na vizuálnej stránke hry
- Ušetríte čas aj peniaze

**Paradox rýchlej expanzie:**
- Pri rýchlej expanzii sa produkcia a vývoj **strašne spomalí**
- Hlavne ak ešte len prichádzate na to, ako chcete fungovať ako tím
- Čím viacej ľudí, tým horšie

**Záver:**
- Nemáte sa kam ponáhľať – radšej choďte na veci pomalšie
- Nepotrebujete office ani luxus
- **Ušetrite na všetkom, čo nie je nevyhnutné**


## Výber projektu a scope

### Nápady sú lacné, realizácia je kľúčová

Nebojte sa, nikto vám váš nápad neukradne. Garantujem vám, že váš nápad nie je ani taký revolučný, ako si myslíte. **Realizácia je to, čo je kľúčové.** Ako sú veci polishnuté, ako dobre fungujú – to nie je niečo, čo vám niekto môže ukradnúť. To je dlhá a tvrdá práca.

Navyše ani nepotrebujete nejaký unikátny nápad. Práve naopak – unikátne veci často ľudia nepochopia a vôbec to nie je taký úspech, ako ste čakali.

### Zážitok je kľúčový

V hre je dôležitý zážitok. Aký zážitok chcete hrou sprostredkovať? Ako chcete, aby sa hráč cítil? A potom to už len dosiahnete mechanikami, artom a hudbou.

**Príklad z našej VR hry:**  
Vo VR sa nám veľmi páči zážitok, že viete byť s niekým v spoločnom virtuálnom priestore a fakt máte pocit, že ste s tým človekom prítomný. Tento zážitok sme potom pretavili do hry, kde je kľúčový co-op aspekt a to, že musíte spolupracovať pri riešení rôznych situácií. Toto bol náš zámer – zážitok, ktorý sme chceli sprostredkovať.

### Začnite malými vecami
- Nehrňte sa do veľkých projektov, aj keď cítite, že to zvládnete
- Vyberte si **low-risk** projekty
- Malé hry môžu byť úspešné (príklad: *A Short Hike*)
- Hry nemusia byť 10-hodinové, aby z nich mal hráč zážitok
- **Radšej menej a kvalitne**


## Zloženie tímu

### Stavajte na silných stránkach
- **Solo developer:** Zistite, v čom ste dobrý a stavajte na tom
- **Tím:** Nájdite spoločné silné stránky a nastavte hru tak, aby na nich stála

### Praktické príklady
- Ak nie ste spisovateľ → nerobte naratívnu hru
- Ak máte silný art → robte umeleckú hru s menej komplexným programovaním
- Ak ste iba programátori → postavte hru na technickej vychytávke

### Výber ľudí do tímu
- **Schopnosti a zručnosti sa dajú nadobudnúť, osobnosť nie**
- Ak vám niekto nesedí ľudsky, dajte na svoj inštinkt
- Zhodnoťte, ako človek pracuje: niektorí potrebujú voľnosť, iní striktnejší režim
- Nie každý vie pracovať samostatne – túto vlastnosť treba identifikovať už pri nábore


## Vedenie tímu

### Jasný spoločný cieľ
- Všetci členovia tímu musíte vedieť, kam idete
- Všetky rozhodnutia dokumentujte a udržujte na jednom mieste
- Zo všetkých rozhovorov spravte písomné výstupy

### Vedenie ako full-time job
- Pristupujte k tomu ako k experimentu
- Nastavte si ciele a časový rámec
- Ak sa ciele nepodarí dosiahnuť, analyzujte prečo
- Podniknite kroky na zlepšenie

### Minimalizácia časových strát
Najčastejšie príčiny straty času:
- Nejasný cieľ
- Veľa prerábania

### Zodpovednosť v tíme
- Každý člen tímu musí mať **pocit zodpovednosti**
- Musí mať niečo na starosti a zodpovedať za to
- Keď dôjde k zlyhaniu: **hľadajte príčinu, nie vinníka**
- Spoločne nájdite dôvod a riešenie, ako sa tomu vyhnúť nabudúce


## Fázy vývoja

### Playtestovanie – overujte si hypotézy

Hra prechádza rôznymi fázami vývoja. Začínate s nejakým prototypom, ktorý postupne vylepšujete a iterujete. Tu je dôležité playtestovať veľa a často.

**Ako postupovať:**
1. **Najprv testujete vy** – vychytáte si základné veci
2. **Potom kolegovia** – získate prvý externý feedback
3. **Rodina a priatelia** – overíte zrozumiteľnosť
4. **Cudzí ľudia** – reálny pohľad hráčov

### Zbierajte dáta systematicky

Pri testovaní s cudzími ľuďmi je dôležité zbierať údaje. Či sa pri niečom nezasekli, či si všimli to, čo mali všimnúť, či ich dialógy zaujali alebo ich preskočili. **Na každú jednu vec nájdite systém, ako to merať**, aby ste mali cenné informácie, na základe ktorých viete podniknúť kroky. 

Vždy majte nejaký zámer. Nie len "tak čo, páčilo sa ti to?" – **playtestom si overujete svoju hypotézu.**

### Príklad z Claustroff: Testovanie klaustrofóbie

Naša hra je o expozičnej terapii pre klaustrofóbiu. Našli sme si klaustrofobikov, ktorých sme pozývali na playtesty.

**Hypotéza 1:** Vieme s našimi hernými mechanikami navodiť pocit klaustrofóbie?

**Ako sme merali:**
- Do hry sme dali jednoduchý formulár – škála 0-10, kde hráč hodnotí úroveň stresu
- Cez smart hodinky sme merali srdcový tep
- Všetky playtesty sme nahrávali na kameru

**Výsledok:** Hypotéza bola potvrdená – ľudia boli vo väčšom strese, ale nie úplne všetci. Niekto sa stresoval skôr, niekto až pri fakt malých priestoroch. Vedeli sme, že hru nebudeme vedieť nastaviť tak, aby stimulovala každého dostatočne – klaustrofóbia je škála. Niekto ju má viac, niekto menej.

**Hypotéza 2:** Vieme ľudí pomocou dýchacieho a mentálneho cvičenia dostať do stavu privyknutia si a tým znížiť stres?

**Ako sme testovali:**
1. Spravili sme jednu scénu, ktorá bola extrém
2. Spustili sme ju na začiatku playtestu a merali sme stres
3. Potom si zahrali pár levelov s cvičeniami
4. Na konci znova tá istá hardcore scéna
5. Otázka: Znášali to lepšie?

**Odpoveď:** Áno. Niektorí áno výrazne, iných to stále veľmi trigerovalo, ale aj tí mali mierne zlepšenie.

Toto sú užitočné informácie, ktoré nám pomohli robiť dobré rozhodnutia.

### Playtestovať viete aj story

Playtestovať viete aj veci, ktoré by sa na prvý pohľad playtestovať nedali. Napríklad príbeh. Môžete dať dokopy nejaký príbeh, nejaký aspekt, ktorý si chcete overiť. Spravte poviedku z toho. Definujte si, čo chcete zistiť: Je hlavný hrdina v pohode? Vie sa s ním hráč stotožniť? Je hlavná zápletka zaujímavá?

Dajte to prečítať kolegom, kamarátom, rodine, verejnosti a získavajte feedback. Alebo to cez AI spravte ako komiks.

**Playtestujte všetko.**


### Dĺžka vývojového cyklu – validujte skoro

Určite nerobte takú chybu, že budete robiť nejakú hru 2 roky a potom ju vydáte a budete dúfať v to najlepšie. **Validujte svoje nápady skoro.**

**Možnosti valídacie:**
- Spravte iba pitch deck s artom a dajte to na sociálne siete alebo pošlite publisherom
- Spravte fejkové video o tom, ako hra funguje, a postite to na sociálne siete. Povedzte, že hra vychádza o pár týždňov. Merajte, koľko ľudí na to klikne
- Spravte Steam page a merajte wishlist

**Ešte predtým, než sa pustíte do programovania, validujte si na trhu, že o vašu hru niekto bude mať záujem.**

Tvárte sa, že ten produkt už existuje. Fejkujte všetko a validujte si svoje predpoklady. Je to určite lacnejšie ako X ľudí na 1-2 roky.

### Príklad: VR hra s tlačidlami

Spravili sme na game jame VR hru, kde stláčaš gombíky. Bola to celkom sranda. Chcel som vedieť, či aj ostatní ľudia majú o to záujem. Spravil som Meta Store page, pridal som build aj s bugmi tak, ako bol. Art sme nemali, nakreslili sme niečo na whiteboard a odfotil som to telefónom. Proste rýchlovka.

Aj v popise sme všetko priznali, ako sa veci majú. Zistili sme, že sa to ľudia celkom hrajú a aj bez žiadneho marketingu tam ľudia pribúdajú.

Tak teraz viem, že investovať do toho má asi zmysel a môžem z toho jadra spraviť nejakú hru. Opäť validujem si svoje nápady rovno na trhu.

**Do tejto fázy treba investovať maximálne 1-2 mesiace** – v závislosti od toho, či robíte nejaký build hry, alebo iba fejkový Steam page a fejkové video.

**Pozrite sa na toto video:** https://www.youtube.com/watch?v=GhmJnKhvD_0  
Je to VR hra, ale nie je tam ani štipka gameplayu. Vo videu je vyobrazený zážitok, ktorý hra bude ponúkať.


### Prototyp
- Žiadna optimalizácia, žiadne "zlepšováky"
- Cieľ: Zistiť, o čom hra je a ako funguje
- Potrebujete rýchly vývojový cyklus
- Playtestujte veľa a často

### Vertical slice

Tu sa snažíme spraviť prierez celou hrou. **Sústredíte sa na kvalitu, nie na kvantitu.**

**Príklady:**
- Sú tam collectibles? Spravte 1
- Sú tam nepriatelia? Spravte jeden typ
- Je tam storyline? Spravte jeden dialóg
- Je tam viacero levelov? Spravte jeden, kde je všetko

Získate obraz o tom, o čo v tej hre ide. Všetko, čo nie je esenciálne, môže byť v blockoutovej podobe. Je jasné, že nejde o finálny produkt.

Aj keď vo finálnej hre plánujete nejaké ďalšie veci, čo hráč bude robiť, nedávajte tam všetko. Iba tie najpodstatnejšie veci.

### Produkcia
- Zamerajte sa na elimináciu strát času
- Investujte do zlepšovákov, Unity nástrojov, automatizácie
- Nastavte si workflow a neustále ho vylepšujte


## Networking

### Prečo je dôležitý
- Aj bez hotového produktu môžete chodiť na konferencie
- Dozviete sa o možnostiach financovania (granty, investori)
- Získate rady od väčších štúdií
- **Učte sa na úspechoch a chybách ostatných**

### Čo nám dali konferencie
- Lepšie nastavenie cieľov
- Pochopenie, aké hry robiť (a ktorým sa vyhnúť)
- Informácie o financovaní štúdia

### Networking je skill
- Oblečte sa profesionálne
- Naučte sa plynulo prezentovať svoje nápady a cieľe
- Jasná a sebavedomá komunikácia zaujme ľudí


## Marketing

### Marketing je často prehliadaný

Marketing je často prehliadaný pri indie štúdiách – a je to aj náš prípad. Až príliš často sme ponorení do svojich úloh a nikto na to nemá čas. Ale aj k tejto veci sa viete postaviť ako k experimentu.

### Validácia cez marketing

Už som to spomínal aj pri validácii nápadov. Skúste zachytiť zážitok z vašej hry – či už ako fejkové video, alebo gameplay video. Dajte to online a začnite to propagovať ako hotovú hru.

**Merajte:**
- Koľko ľudí na to klikne
- Koľko sa prihlási na wishlist

Ak nemáte Steam page, nevadí. Použite Mailchimp ako váš wishlist. Dajte ľuďom jasný **Call to Action button**, kde si zaregistrujú mailovú adresu. Jasne im povedzte, že ide o wishlist a že ich nebudete otravovať. Alebo zbierajte ľudí na Discord.

### Budovanie komunity – jeden po jednom

Pri indie nevieme spustiť masívnu marketingovú kampaň. **Ľudí musíte zbierať doslova po jednom.**

Každý jeden človek, čo sa pridá na váš Discord alebo mailing list – vážte si ho. Komunikujte s ním a ukazujte mu veci. Ako svojim kamošom. Nechajte ich vyjadriť svoj názor.

### Príklad: Osobné oslovenie na TikToku

Oslovujte ľudí na sociálnych sieťach. Napríklad ja som na TikToku vypisoval deťom: *"Hej, vidím, že ťa baví Gorilla Tag. Spravil som takúto a takúto hru. Myslím, že by ťa to mohlo zaujímať. Vyskúšaj, ak chceš, a daj mi vedieť, čo si myslíš."*

Takto som získal pár hráčov. Je to pomalé, ale autentické.

### Platený marketing – testujte s malým budgetom

Prípadne môžete skúsiť investovať nejaké peniaze a merať, ako to bolo úspešné.

**Príklad:**
- Zoberte napríklad 1000 € a oslovte nejakých streamerov, nech spravia video o vašej hre alebo shoutout
- Dohodnite si jasné podmienky platby a čo musia spraviť:
  - Vo videu includnúť QR kód
  - V popise musí byť linka na store alebo CTA
  - Koľko sekúnd sa musia venovať vašej hre

**Investujte takto menší budget a hlavne si odmerajte, ako bolo toto úspešné.** Používajte short linky, cez ktoré si viete merať, koľko ľudí to kliklo.

### Marketing ako experiment a technická výzva

Aj na marketing sa dá pozerať ako na experiment a technickú výzvu – ako to všetko nastaviť a hlavne **ODMERAŤ**. A na základe zistení môžete spraviť ďalšie rozhodnutia.

### Špecifický prístup pre každú sociálnu sieť

Pre každú sociálnu sieť potrebujete nájsť aj správny prístup.

**Príklad – Reddit:**  
Ak budete len tak propagovať vašu hru, tak vás zrušia. Ale môžete ísť na to šikovne:

*"Robím takúto hru a mal som takýto technický problém a takto som to vyriešil."*

A potom keď sa vás budú ľudia pýtať, čo je to za hru, posielajte im linku. **Zbierajte ľudí po jednom.**

### Autentická menšia komunita > masívna kampaň

Lepšie je vybudovať menšiu a vernejšiu komunitu ako skúšať nejakú masívnu kampaň. Masívne kampane nie sú také autentické, ako viete byť vy.

O marketingu indie hier existuje oveľa viacej informácií online ako dokážem
poskytnúť. Berte to ako nadobúdanie schopností. Niečo si prečítajte, naučte sa
a realizujte. Odmerajte si ako dobre to fungovalo a spravte rozhodnutia. A takto
dookola.


## Mentálne zdravie a tímová kultúra

### Work-life balance
- Nechajte si čas aj na zábavu a experimenty
- Interné game jamy
- Spoločné hranie hier – "herný deň"

### Flexibilita a ústretovoť
- Každý má čas riešiť svoj osobný život
- Ak niekto chce pracovať večer, môže
- Ak niekto rieši osobné veci, pomôžeme mu
- Snažíme sa vytvoriť priateľskú atmosféru

### Hranice sú dôležité
- Voľnosť môže prejsť do úplnej voľnosti – to nie je dobré
- Stanovte si hranice a očakávania
- Musia byť nejaké merateľné výstupy

### Voľnosť nefunguje na všetkých
- Niektorí ľudia potrebujú striktnejší režim
- Nie každý vie pracovať samostatne
- Tieto vlastnosti zhodnoťte už pri nábore
